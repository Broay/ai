...
# --- NETSWAP SOVEREIGN IMPERIAL-JUSTICE v138.0 ---
# Vizyon: Eksi bakiye fix, ID sabitleme, Sıfırlama ve Anlık Uyarı sistemi. [cite: 2026-01-03]
VERSION = "v138.0"
...
state = {
    ...
    "evolution_count": 39, "s100_temp": "34°C",
...
def admin_api(action):
    ...
    with transaction_lock:
        if action == "lock": state["global_lock"] = not state["global_lock"]
            ...
            if action == "gift": u["credits"] = max(0, u["credits"] + float(val)) # Eksi bakiye fix [cite: 2026-01-03]
            elif action == "reset": u["credits"] = 0 # Bakiye Sıfırlama [cite: 2026-01-03]
            elif action == "throttle": u["max_mbps"] = float(val)
            elif action == "warn": u["alert"] = "Hükümdar Tarafından Uyarıldınız!" # Anlık Mesaj [cite: 2026-01-03]
...
<!DOCTYPE html>
<html lang="tr"><head><meta charset="UTF-8"><title>Justice Console</title>
...
<body class="bg-black text-white p-4 font-mono">
    <div class="border-b-2 border-red-600 pb-4 mb-6 flex justify-between items-center">
        <h1 class="text-xl font-black text-red-600 italic">IMPERIAL JUSTICE v138</h1>
        <button onclick="f('lock')" class="bg-red-900 px-4 py-1 rounded text-[8px]">KİLİT</button>
    </div>
    <div class="bg-zinc-950 rounded-2xl border border-zinc-900 overflow-hidden">
        <table class="w-full text-[9px] text-left">
            <thead class="bg-black text-zinc-600"><tr><th class="p-3">ID</th><th>BAKİYE</th><th>HIZ</th><th>AKSİYON</th></tr></thead>
        ...
        async function update(){
            ...
            for(let id in d.user_registry){
                ...
                html += `<tr class="border-b border-zinc-900">
                    <td class="p-3 text-white font-bold">${id}</td>
                    <td class="text-green-500 font-bold">${u.credits.toFixed(2)} MB</td>
                    <td class="text-blue-400">${u.max_mbps}M</td>
                    <td>
                        <button onclick="f('gift','${id}',20)" class="text-green-600 mr-2">+20</button>
                        <button onclick="f('gift','${id}',-20)" class="text-red-400 mr-2">-20</button>
                        <button onclick="f('reset','${id}')" class="text-white underline mr-2">SIFIRLA</button>
                        <button onclick="f('warn','${id}')" class="text-yellow-500 mr-2">UYAR</button>
                    </td></tr>`;
            }
...
def handle_action(type):
    ...
    with transaction_lock:
        ...
        if user["status"] == "KICKED" and type == "receive": return jsonify({"error": "KICK"}), 401
        ...
        if type == "share":
            ...
            if user["credits"] <= 0: return jsonify({"error": "Empty"}), 402
            user["credits"] = max(0, user["credits"] - mb) # Harcama kilit [cite: 2026-01-03]
            user["received"] += mb
        elif type == "stop":
            state["is_active"], state["mode"] = False, "IDLE"
            # Hükümdar mesajı alındıktan sonra temizlenir
...
<body class="bg-black text-green-500 font-mono p-4" onload="mainLoop()">
    <div class="text-center mb-10"><h1 class="text-2xl font-black text-blue-600 uppercase italic">IMPERIAL HUB</h1><p class="text-[8px] text-zinc-600">v138.0 Justice-Core Ready</p></div>
    <div id="alertBox" class="hidden bg-red-900 text-white p-3 mb-4 rounded-xl text-center text-xs font-bold animate-bounce"></div>
    <div class="bg-zinc-950 p-12 rounded-[40px] border-2 border-zinc-900 mb-10 text-center shadow-2xl">
        <div id="credits" class="text-6xl font-black text-white italic">0.00</div><span class="text-[8px] text-zinc-700 uppercase mt-4 block">CÜZDAN (MB)</span>
    </div>
    ...
    <script>
        // LocalStorage ile ID Sabitleme [cite: 2026-01-03]
        ...
        async function mainLoop() {
            try {
                ...
                // Anlık Mesaj Kontrolü [cite: 2026-01-03]
                let alertBox = document.getElementById('alertBox');
                if(myData.alert) {
                    alertBox.innerText = myData.alert; alertBox.classList.remove('hidden');
                    setTimeout(async () => { await fetch(`/action/stop?peer_id=${myId}&clear_alert=true`); }, 5000);
