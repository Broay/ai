...
# --- NETSWAP SOVEREIGN AMNESTY v140.0 ---
# Vizyon: Yasaklıları geri açma (Unban), Dinamik miktar ve Mutlak müdahale. [cite: 2026-01-03]
VERSION = "v140.0"
...
state = {
    ...
    "evolution_count": 41, "s100_temp": "34°C",
...
def admin_api(action):
    ...
    with transaction_lock:
        ...
        # UNBAN MEKANİZMASI [cite: 2026-01-03]
...
<!DOCTYPE html>
<html lang="tr"><head><meta charset="UTF-8"><title>Sovereign Amnesty Console</title>
...
<body class="bg-black text-white p-4 font-mono">
    <div class="border-b-2 border-red-600 pb-4 mb-6 flex justify-between items-center">
        <h1 class="text-xl font-black text-red-600 italic uppercase">Sovereign Amnesty v140</h1>
    ...
    <div class="bg-zinc-950 rounded-2xl border border-zinc-900 overflow-hidden shadow-2xl mb-8">
        <div class="p-3 bg-zinc-900/50 text-[10px] font-bold text-green-500 uppercase italic">Aktif Tebaalar</div>
    ...
    <div class="bg-zinc-950 rounded-2xl border border-red-900 overflow-hidden shadow-2xl">
        <div class="p-3 bg-red-950/20 text-[10px] font-bold text-red-500 uppercase italic">Kara Liste (Banlananlar)</div>
        <table class="w-full text-[9px] text-left">
            <thead class="bg-black text-zinc-600"><tr><th class="p-3">PEER ID</th><th>DURUM</th><th>AKSİYON</th></tr></thead>
        ...
        async function update(){
            ...
            for(let id in d.user_registry){
                ...
                if(d.banned_peers.includes(id)){
                    ...
                    <td><button onclick="f('unban','${id}')" class="text-green-500 underline">ENGELİ KALDIR</button></td></tr>`;
                } else {
...
<body class="bg-black text-green-500 font-mono p-4" onload="mainLoop()">
    <div class="text-center mb-10"><h1 class="text-2xl font-black text-blue-600 uppercase italic">IMPERIAL HUB</h1><p class="text-[8px] text-zinc-600">v140.0 Amnesty-Ready Core</p></div>
